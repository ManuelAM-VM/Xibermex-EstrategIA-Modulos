...
function StatCard({
  ...
  return (
    ...
    >
      ...
      <p style={{ fontSize: '28px', fontWeight: '700', color: color || '#e2e8f0' }}>{value}</p>
    </div>
  )
}

function ColaboradorCard({ stats }: { stats: ColaboradorStats }) {
  const initials = stats.nombre
    .split(' ')
    .slice(0, 2)
    .map((n) => n[0])
    .join('')
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
        ...
        />
        <span style={{ fontWeight: '600', color: '#e2e8f0' }}>{stats.nombre}</span>
      </div>
...
function Row({
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      ...
      <span style={{ fontSize: '13px', fontWeight: '600', color: valueColor || '#e2e8f0' }}>
        {value}
      </span>
    </div>
  )
        ...
        {loading ? (
          ...
          <>
            ...
            <div style={{
              ...
              <StatCard label="Pendientes"    value={data.pendientes}   color="#f59e0b" />
